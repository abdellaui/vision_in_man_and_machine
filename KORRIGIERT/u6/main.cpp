...
-Für jede nicht triviale Permutation von Größenskalierung und Rauschstärke ein Bild erzeugt.
	-Skalierungsfaktor 	von 0.1 bis -2.0 mit 0.1  Schritten (18 Iterationen)
	-Rauschstärke 		von 0.0 bis  1.0 mit 0.05 Schritten (20 Iterationen)

//@Note: Das ist weitaus mehr Arbeit als verlangt war, aber ist natürlich sehr schön, dass ihr euch so viel Mühe gegeben habt.
...
Die maximale Rauschstärke, bei dem die Nase auffindbar war, ist 0.25.
...
je größer das Template umso weniger Fehlmatches,
je kleiner das Template umso mehr Treffer kriegt man, jedoch wahrscheinlich mit vielen Fehltreffer.
Was natürlich auch stark an dem zu analysierenden Bild abhängt. 
Wenn man beispielsweise Knöpfen in einem Bild detektieren möchte, dann wäre es nicht geeignet ein großes Template zu wählen,
da hierbei auch der Hintergrund betrachtet wird.
In den meisten Fällen sollte ein Template im mittleren Bereich liegen, sprich nicht zu groß auch nicht zu klein.
Daher gibt keine genau richtige Antwort, wie groß ein Template sein sollte. 
Das Template sollte so groß sein , wie es nur sein muss.
Das heisst unnötige Pixelen sollten vermieden werden.
...
int pragma::main(int argc, char *argv[])
{
	...
	pragma::REAL scaleSubImage = 1.8;
	pragma::REAL noiseStrength = 0.4;
	...
	pragma_assert(posSubImageX>=0);
	pragma_assert(posSubImageY>=0);
	pragma_assert(widthSubImage>0);
	pragma_assert(heightSubImage>0);
	...
	// @Note: Hier processImage aufzurufen wäre schöner, weil die Funktion extra dafür geschrieben wurde, dass man die einzelnen nicht aufrufen muss.
	...
	testImage->writePNG("processedImg.png");
	...
	//@Note: Die Funktion hat sonst fast niemand gefunden, aber ist natürlich die eleganteste Lösung überhaupt.
